...
def build_lineage_markdown(ctx, max_steps: int, lang: str = "en") -> str:
    ...
    joined = build_step_join(ctx.game_logs, ctx.transitions, ctx.replay_entries)
    ...
    summary_rows = [
        ...
        ["replay_rows", len(ctx.replay_entries)],
        ["replay_format", replay_format or ("(없음)" if is_ko else "(missing)")],
    ...
    for row in joined[:max_steps]:
        ...
        step_rows.append(
            [
                ...
                row["replay"].get("action_id") if row["replay"] else "",
                yes_label if db_log and db_log.state_summary else "",
                yes_label if jsonl and jsonl.get("model_info") else "",
                yes_label if row["replay"] and row["replay"].get("model_info") else "",
    ...
    sequence_lines = [
        ...
        '    participant Replay as "replay JSON"',
    ...
    for row in joined[: min(max_steps, 8)]:
        ...
        if jsonl:
            ...
            action = str(replay.get("action_id", ""))
        ...
        sequence_lines.extend(
            [
                ...
                f'    Service->>Replay: append entry step={step_label}',
...
```"""
    ...
    sections = [
        ...
        markdown_table(
            ["round", "step", "actor_id", "db_action", "jsonl_action", "replay_action", "db_summary", "jsonl_model", "replay_model", "jsonl_mask"]
            if not is_ko else
            ["round", "step", "actor_id", "DB 액션", "JSONL 액션", "Replay 액션", "DB 요약", "JSONL 모델", "Replay 모델", "JSONL 마스크"],
            step_rows or [["", "", "", "", "", "", "", "", "", ""]],
        ),
        "",
        ...
        bullet_list(
            [
                ...
                + " replay JSON이 있으면 같은 키로 함께 대조합니다."
                if is_ko else
                "This report joins DB, JSONL, and replay rows by (round, step, actor_id). A canonical step_id does not exist yet.",
