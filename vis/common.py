...
def discover_transition_files(explicit_paths: Sequence[str] | None = None) -> list[Path]:
    if explicit_paths:
        ...
        for path_str in explicit_paths:
            ...
            if path.is_file():
                ...
                resolved.extend(sorted(path.glob("transitions_*.jsonl")))
    ...
    per_game_files = sorted(per_game_dir.rglob("*.jsonl")) if per_game_dir.exists() else []
    legacy_files = sorted(DEFAULT_LOG_DIR.glob("transitions_*.jsonl"))
    return sorted(dict.fromkeys([*per_game_files, *legacy_files]))
...
def sort_transition_records(records: Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
    def _sort_key(record: dict[str, Any]) -> tuple[Any, ...]:
        round_value = extract_transition_round(record)
        step_value = extract_transition_step(record)
        timestamp = _safe_ts(record.get("timestamp"))
        source_file = str(record.get("_source_file") or "")
        source_line = _safe_int(record.get("_source_line")) or 0
        return (
            round_value is None,
            round_value if round_value is not None else 10**9,
            step_value is None,
            step_value if step_value is not None else 10**9,
            timestamp,
            source_file,
            source_line,
        )
...
def load_replay_payload(game_id: str | None) -> tuple[Path | None, dict[str, Any] | None]:
    if not game_id:
        return None, None
    replay_path = DEFAULT_LOG_DIR / "replay" / f"{game_id}.json"
    if not replay_path.is_file():
        return replay_path, None
    try:
        return replay_path, json.loads(replay_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return replay_path, None


def extract_replay_entries(payload: dict[str, Any] | None) -> list[dict[str, Any]]:
...
def extract_replay_model_info(entry: dict[str, Any]) -> dict[str, Any] | None:
...
def load_context(
    *,
    ...
    filtered_transitions = sort_transition_records(filtered_transitions)
    room = load_game_session(resolved_db_url, effective_game_id)
    game_logs = load_game_logs(resolved_db_url, effective_game_id)
    replay_path, replay_payload = load_replay_payload(effective_game_id)
    replay_entries = extract_replay_entries(replay_payload)
    ...
    elif replay_payload is None:
        warnings.append("Replay JSON was not found or could not be parsed for the selected game_id.")
...
def step_key_from_replay_entry(entry: dict[str, Any]) -> tuple[int, int, str]:
    return (
        _safe_int(entry.get("round")) or -1,
        _safe_int(entry.get("step")) or -1,
        str(entry.get("actor_id") or ""),
    )
...
def build_step_join(
    ...
    replay_entries: Sequence[dict[str, Any]] | None = None,
) -> list[dict[str, Any]]:
    ...
    replay_map = {step_key_from_replay_entry(entry): entry for entry in (replay_entries or [])}
    keys = sorted(set(db_map) | set(jsonl_map) | set(replay_map))
    joined = []
    for key in keys:
        joined.append(
            {
