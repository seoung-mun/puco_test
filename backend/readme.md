...
- bot-only 게임에서 host spectator를 허용
...
형식은 현재 `backend-replay.v1` 입니다.  
`PuCo_RL/logs/replay/*.json` 계열과 비슷한 읽기 경험을 유지하지만, backend 런타임 메타데이터와 최종 점수 요약을 추가로 담습니다.

저장되는 값:

- top-level 메타
  - `format`, `game_id`, `title`, `status`, `host_id`, `players`, `model_versions`, `initial_state_summary`
- `entries`
...
`vis/`는 `game_logs` + `JSONL` + `replay JSON`을 읽어 Markdown 리포트를 만듭니다.
