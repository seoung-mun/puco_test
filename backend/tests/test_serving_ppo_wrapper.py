...
def test_serving_ppo_wrapper_requires_matching_runtime_obs_dim(tmp_path):
    checkpoint_path = tmp_path / "PPO_PR_Server_20260408_120000_step_100.pth"
    agent = Agent(obs_dim=293, action_dim=200)
    torch.save(agent.state_dict(), checkpoint_path)

    wrapper = PPOWrapper(str(checkpoint_path), obs_dim=293)

    obs = torch.zeros(1, 293)
    mask = torch.zeros(1, 200)
    mask[0, 15] = 1.0

    action = wrapper.act(obs, mask)

    assert wrapper._expected_obs_dim == 293
    assert action == 15
