...
- Returns 200 with {"status": "degraded"} when PostgreSQL is down
- Returns 200 with {"status": "degraded"} when Redis is down
...
def mock_serving_health(monkeypatch):
    monkeypatch.setattr(
...
class TestHealthEndpointSuccess:
    ...
    def test_health_includes_serving_check_metadata(self, client):
        response = client.get("/health")
        serving = response.json()["checks"]["serving"]
        assert serving["status"] in {"ok", "degraded"}
        assert "artifact_name" in serving
        assert "metadata_source" in serving


class TestHealthEndpointDegraded:
    def test_health_returns_503_when_postgresql_down(self, client, monkeypatch):
        """Health endpoint must keep returning 200 when PostgreSQL is unreachable."""
        def bad_session():
            mock = MagicMock()
    ...
    def test_health_returns_503_when_redis_down(self, client, monkeypatch):
        """Health endpoint must keep returning 200 when Redis is unreachable."""
    ...
    def test_health_postgresql_ok_redis_down_returns_503(self, client, monkeypatch):
        """Even if only Redis is down, /health must still return 200."""
        mock_redis = AsyncMock()
