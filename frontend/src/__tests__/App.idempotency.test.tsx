...
function makeSettlerState(stateRevision = 7, actionMask: number[] = SETTLER_TEST_MASK) {
  const basePlayer = {
    display_name: 'Alice',
    display_number: 1,
    is_governor: true,
...
describe('channelAction idempotency', () => {
  ...
  it('generates a new intent_id per click and sends the latest revision', async () => {
    render(<App />);
    ...
    fireEvent.click(screen.getByRole('button', { name: /face-up-coffee/ }));

    await waitFor(() => {
      expect(actionFetchCalls).toHaveLength(1);
    });

    await waitFor(() => {
      expect(screen.getByRole('button', { name: /face-up-coffee/ })).toBeTruthy();
    });
    fireEvent.click(screen.getByRole('button', { name: /face-up-coffee/ }));

    await waitFor(() => {
      expect(actionFetchCalls).toHaveLength(2);
    });

    expect(actionFetchCalls[0].body?.payload?.action_intent_id).toBe('intent-1');
    expect(actionFetchCalls[1].body?.payload?.action_intent_id).toBe('intent-2');
    expect(actionFetchCalls[0].body?.payload?.expected_state_revision).toBe(7);
    expect(actionFetchCalls[1].body?.payload?.expected_state_revision).toBe(7);
  });

  it('does not retry when the server responds with 409 stale_state', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        const url = String(input);
