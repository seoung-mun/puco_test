...
def rollout_worker(rank, conn, shared_bufs, obs_dim, action_dim, opponent_pool):
    """
    Persistent environment worker for HPPO.
    ...
    local_agent = HierarchicalAgent(obs_dim=obs_dim, action_dim=action_dim)
    local_opponent = HierarchicalAgent(obs_dim=obs_dim, action_dim=action_dim)
    local_agent.eval()
    local_opponent.eval()
...
def train():
    ...
    run_name = f"HPPO_PR_Server_{int(time.time())}"
    ...
    print(f"===============================================")
    print(f"🚀 Starting HPPO Multiprocessing Train Server")
