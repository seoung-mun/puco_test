...
class PhasePPOAgent(nn.Module):
    """
    PhasePPO Actor-Critic with phase-conditioned separate actor heads.
    ...
    Paper contribution: Puerto Rico's natural phase structure allows
    us to assign specialized actor heads for each distinct phase.
    """
    NUM_PHASES = 9  # Phase IntEnum 0..8

    def __init__(self, obs_dim: int, action_dim: int = 200,
                 hidden_dim: int = 512, num_res_blocks: int = 3):
        super(PhasePPOAgent, self).__init__()
