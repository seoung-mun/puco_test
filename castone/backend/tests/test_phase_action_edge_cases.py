...
class TestPayloadValidation:
    def test_missing_action_index_returns_400(self, client, db):
        ...
        _start(client, game.id, _bearer(user.id))
    ...
    def test_out_of_range_action_returns_400(self, client, db):
        ...
        res = _action(client, game.id, 999, _bearer(user.id))
        assert res.status_code == 400
    ...
    def test_masked_action_rejected_returns_400(self, client, db):
        ...
        user = _make_user(db)
        game = _make_game(db, [str(user.id), "BOT_random", "BOT_random"])
        start_res = _start(client, game.id, _bearer(user.id))

        invalid_idx = _first_invalid(start_res["action_mask"])
        res = _action(client, game.id, invalid_idx, _bearer(user.id))
        assert res.status_code == 400
    ...
    def test_negative_action_index_returns_400(self, client, db):
        ...
        res = _action(client, game.id, -1, _bearer(user.id))
        assert res.status_code == 400
...
class TestRoleSelectionPhase:
    def test_valid_role_selection_returns_200(self, client, db):
        ...
        res = _action(client, game.id, valid_role, _bearer(user.id))
...
class TestSettlerPhase:
    ...
    def _enter_settler(self, client, game, user):
        """Start game, select Settler role (action 0), return response."""
        start_res = _start(client, game.id, _bearer(user.id))
        mask = start_res["action_mask"]
        # Role.SETTLER == 0
        if mask[0] == 0:
            pytest.skip("Settler role not available (may be taken or not in this setup)")
        return _action(client, game.id, 0, _bearer(user.id))

    def test_settler_role_select_enters_phase(self, client, db):
        user = _make_user(db)
        game = _make_game(db, [str(user.id), "BOT_random", "BOT_random"])
        res = self._enter_settler(client, game, user)
        assert res.status_code == 200
    ...
    def test_settler_valid_plantation_action(self, client, db):
        ...
        res2 = _action(client, game.id, settler_valid, _bearer(user.id))
    ...
    def test_out_of_range_action_in_settler_phase_blocked(self, client, db):
        ...
        res2 = _action(client, game.id, 999, _bearer(user.id))
...
class TestBuilderPhase:
    ...
    def _enter_builder(self, client, game, user):
        start_res = _start(client, game.id, _bearer(user.id))
        mask = start_res["action_mask"]
        # Role.BUILDER == 2
        if mask[2] == 0:
            pytest.skip("Builder role not available")
        return _action(client, game.id, 2, _bearer(user.id))

    def test_builder_role_select_enters_phase(self, client, db):
        user = _make_user(db)
        game = _make_game(db, [str(user.id), "BOT_random", "BOT_random"])
        res = self._enter_builder(client, game, user)
        assert res.status_code == 200
        mask = res.json()["action_mask"]
    ...
    def test_builder_valid_build_action(self, client, db):
        ...
        res2 = _action(client, game.id, builder_valid, _bearer(user.id))
        assert res2.status_code == 200
...
class TestTraderPhase:
    ...
    def _enter_trader(self, client, game, user):
        start_res = _start(client, game.id, _bearer(user.id))
        mask = start_res["action_mask"]
        # Role.TRADER == 4
        if mask[4] == 0:
            pytest.skip("Trader role not available")
        return _action(client, game.id, 4, _bearer(user.id))

    def test_trader_role_select_enters_phase(self, client, db):
        user = _make_user(db)
        game = _make_game(db, [str(user.id), "BOT_random", "BOT_random"])
        res = self._enter_trader(client, game, user)
        assert res.status_code == 200
    ...
    def test_trader_sell_action_rejected_when_masked(self, client, db):
        ...
        res2 = _action(client, game.id, 41, _bearer(user.id))
        assert res2.status_code == 400
    ...
    def test_trader_pass_accepted(self, client, db):
        ...
        res2 = _action(client, game.id, 15, _bearer(user.id))
        assert res2.status_code == 200
...
class TestCaptainPhase:
    ...
    def _enter_captain(self, client, game, user):
        start_res = _start(client, game.id, _bearer(user.id))
        mask = start_res["action_mask"]
        # Role.CAPTAIN == 5
        if mask[5] == 0:
            pytest.skip("Captain role not available")
        return _action(client, game.id, 5, _bearer(user.id))

    def test_captain_role_select_enters_phase(self, client, db):
        user = _make_user(db)
        game = _make_game(db, [str(user.id), "BOT_random", "BOT_random"])
        res = self._enter_captain(client, game, user)
        assert res.status_code == 200
    ...
    def test_captain_load_masked_action_blocked(self, client, db):
        ...
        res2 = _action(client, game.id, 44, _bearer(user.id))
        assert res2.status_code == 400
...
class TestMayorPhase:
    ...
    def _enter_mayor(self, client, game, user):
        start_res = _start(client, game.id, _bearer(user.id))
        mask = start_res["action_mask"]
        # Role.MAYOR == 1
        if mask[1] == 0:
            pytest.skip("Mayor role not available")
        return _action(client, game.id, 1, _bearer(user.id))

    def test_mayor_role_select_enters_phase(self, client, db):
        user = _make_user(db)
        game = _make_game(db, [str(user.id), "BOT_random", "BOT_random"])
        res = self._enter_mayor(client, game, user)
        assert res.status_code == 200
    ...
    def test_mayor_valid_colonist_placement(self, client, db):
        ...
        res2 = _action(client, game.id, mayor_valid, _bearer(user.id))
        assert res2.status_code == 200
...
class TestConsecutiveActions:
    ...
    def test_cannot_replay_already_consumed_action(self, client, db):
        ...
        if phase_mask[role_action] == 0:
            ...
            res2 = _action(client, game.id, role_action, _bearer(user.id))
            assert res2.status_code == 400
