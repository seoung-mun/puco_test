...
`.env`는 저장소에 커밋하지 않습니다. 팀 공유용 `.env`를 받은 뒤 저장소 루트에 두고 실행하세요.
...
- 기본 서빙 체크포인트는 `PuCo_RL/models/`에 함께 커밋됩니다.
- 체크포인트와 같은 basename의 `.json` sidecar 메타데이터도 같이 유지합니다.
- 팀원이 clone 후 `.env`만 맞추면 같은 모델 파일을 그대로 사용합니다.
